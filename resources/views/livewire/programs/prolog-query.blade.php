<div x-cloak x-data="{modalOpen:false}" class="w-full p-4 text-center border border-gray-200 rounded-md my-4">
    ...
    <x-textarea class="outline-none bg-blue-50 w-full py-2 px-4 rounded-md shadow" name="content" wire:model="prologQuery.content" wire:keydown.debounce.200ms="save()" rows="3"></x-textarea>
    <x-error class="text-red-500" field="prologQuery.content" />
    <div class="h-8 mt-3">
        <button @click="window.evaluate()" class="inline-flex items-center text-sm py-1 px-3 border border-gray-200 rounded-lg shadow-sm hover:shadow-md cursor-pointer float-right" type="button">
