...
(function(pl) {
  var width = function(ctx, font, margin, padding) {
    ...
    return (
      Math.max(
        ctx.measureText(this.text_goal).width,
        ctx.measureText(this.text_substitution).width
      ) +
      2 * margin +
      2 * padding
    );
  ...
  var get_states_at_level = function(nodes, level) {
    ...
    for (var i = 0; i < nodes.length; i++)
      children = children.concat(
        get_states_at_level(nodes[i].children, level - 1)
      );
    return children;
  };
  ...
  var clear_substitution = function(subs) {
    ...
    for (var x in subs.links) {
      ...
      if (!pl.type.is_variable(link) || link.id != x) links[x] = link;
    }
  ...
  var init = function(thread, max, canvas, styles, options) {
    ...
    set_style(
      styles,
      "font",
      styles["font-size"] + "px " + styles["font-family"]
    );
  ...
  var draw = function(tree, canvas_id, styles) {
    ...
    var canvas =
      typeof canvas_id === "string"
        ? document.getElementById(canvas_id)
        : canvas_id;
    var ctx = canvas.getContext("2d");
    // Maximum width of each level
    var maxwidth = tree.map(function(a) {
      return a.reduce(function(b, c) {
        return b + c.width(ctx, styles["font"], margin_x, padding);
      }, 0);
    }, 0);
    // Width of the image
    var width = maxwidth.reduce(function(a, b) {
      return Math.max(a, b);
    }, 0);
    ...
    for (var i = 0; i < tree.length; i++) {
      ...
      for (var j = 0; j < tree[i].length; j++) {
        ...
        var center =
          offset_x +
          tree[i][j].width(ctx, styles["font"], margin_x, padding) / 2;
        ...
        if (i > 0) {
          ...
          ctx.beginPath();
          ctx.moveTo(
            relative[i - 1][tree[i][j].parent.id_canvas_level],
            offset_y - unifier_height / 2
          );
    ...
    for (i = 0; i < tree.length; i++) {
      ...
      for (j = 0; j < tree[i].length; j++) {
        ...
        if (i > 0) {
          var x1 = Math.abs(
            relative[i - 1][tree[i][j].parent.id_canvas_level] - center
          );
          ...
          ctx.textAlign = "center";
          ctx.fillText(
            tree[i][j].preorder_id,
            center + x3,
            offset_z + font_size / 2 - 1
          );
          offset_z += radius + margin_y;
        }
        // Draw state
        var type_state;
        if (tree[i][j].status && tree[i][j].status === "answer")
          // Answer
          type_state = "answer";
        else if (tree[i][j].children.length == 0 && tree[i][j].goal !== null)
          // Error
          type_state = "error";
        else type_state = "state";
        ...
        ctx.beginPath();
        ctx.rect(
          center - tree[i][j].width(ctx, styles["font"], 0, padding) / 2,
          offset_z,
          tree[i][j].width(ctx, styles["font"], 0, padding),
          unifier_height
        );
        ctx.closePath();
        ctx.stroke();
        ...
        ctx.fillText(
          tree[i][j].text_goal,
          center,
          offset_z + padding + font_size
        );
        ctx.fillText(
          tree[i][j].text_substitution,
          center,
          offset_z + 2 * font_size + 2 * padding
        );
        offset_z += unifier_height;
        // Update offset
        offset_x += tree[i][j].width(ctx, styles["font"], margin_x, padding);
  ...
  if (typeof module !== "undefined") {
    module.exports = function(p) {
      pl = p;
      pl.type.Thread.prototype.draw = function(max, canvas, styles, options) {
        return init(this, max, canvas, styles, options);
      };
      pl.type.Session.prototype.draw = function(max, canvas, styles, options) {
        return this.thread.draw(max, canvas, styles, options);
      };
    };
  } else {
    pl.type.Thread.prototype.draw = function(max, canvas, styles, options) {
